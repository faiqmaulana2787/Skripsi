...
@section('content')
    <div class="container mx-auto p-6">
        <h2 class="text-2xl font-bold mb-4 text-center">Edit Jurusan</h2>

        <form
            action="{{ route('jurusan.update', ['kode_ptn' => $ptn->kode_ptn, 'kode_jurusan' => $jurusan->kode_jurusan]) }}"
            method="POST" class="max-w-lg mx-auto bg-white p-6 rounded shadow">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="jenjang" class="block font-semibold">Jenjang:</label>
                <input type="text" name="jenjang" value="{{ $jurusan->jenjang }}" required
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nilai_jurusan" value="{{ $jurusan->nilai_jurusan }}" required
                    class="border border-gray-300 rounded w-full p-2 focus:ring focus:ring-blue-300">
            </div>
